            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Loan Date
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Return
                            Date</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($loans as $loan)
                        @php
                            $fine = 0;
                            if ($loan->status === 'borrowed' && $loan->loan_date && now()->gt($loan->due_at)) {
                                $daysOverdue = now()->diffInDays($loan->due_at);
                                $fine = $daysOverdue * 70; // Correct fine per day
                            }
                        @endphp
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-red-600 font-semibold">
                                {{ $fine > 0 ? number_format($fine, 2) : '-' }}</td>
