...
<div class="max-w-6xl mx-auto mt-10 px-4">
    ...
    <div class="bg-white shadow rounded-lg p-6">
                ...
                <table class="min-w-full border border-gray-200 rounded-lg overflow-hidden text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b hover:bg-gray-50 transition">
                            <td class="py-3 px-4 font-medium text-gray-800">{{ $loan->book->title ?? 'Unknown Book' }}</td>
                            <td class="py-3 px-4 text-gray-600">{{ $loan->borrowed_at?->format('M d, Y') ?? '-' }}</td>
                            ...
                            <td class="py-3 px-4">
                                @if($loan->status === 'borrowed')
                                    @php
                                        ...
                                        $buttonColor = $daysLeft / $totalDays > 0.66 ? 'bg-green-600 hover:bg-green-700'
                                                       : ($daysLeft / $totalDays > 0.33 ? 'bg-orange-500 hover:bg-orange-600'
                                                       : 'bg-red-600 hover:bg-red-700');
                                    @endphp
                                    <form action="{{ route('loans.return', $loan->book->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="px-4 py-2 text-white rounded-lg transition {{ $buttonColor }}">
                                            Return
                                        </button>
                                    </form>
                                @else
        ...
        <div class="text-center py-10 text-gray-600">
            ...
            <a href="{{ route('home') }}" class="inline-block mt-4 px-5 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">
                Browse Books
            </a>
        </div>
        @endif
